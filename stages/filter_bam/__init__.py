...
def split(args):
    ...
    # one job per barcode-BED combo
    bc_chunks = []
    for cluster_id, d in df.groupby('Cluster'):
        bc_chunks.append({'cluster_bcs': d.Barcode.tolist(), 'cluster_id': cluster_id, '__mem_gb': 8})
...
def main(args, outs):
    ...
    regions = [x.split() for x in open(args.target_regions)]

    for r in regions:
        chrom, start, stop = r[:3]
        start = int(start)
        stop = int(stop)
        bam_iter = in_bam.fetch(chrom, start, stop)
